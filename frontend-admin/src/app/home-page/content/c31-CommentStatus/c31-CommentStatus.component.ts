...
@Component({                                                                
  selector: 'app-c31-CommentStatus',                                                
  templateUrl: './c31-CommentStatus.component.html',                                
  styleUrls: ['./c31-CommentStatus.component.scss'],                                
})                                                                          
export class C31CommentstatusComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '3006').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '3005').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '3003').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C31CommentstatusDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c31-CommentStatus-dialog',                            
  templateUrl: 'c31-CommentStatus-dialog.html',                    
  styleUrls: ['./c31-CommentStatus.component.scss'],               
})                                                        
export class C31CommentstatusDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C31CommentstatusDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(3001 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
