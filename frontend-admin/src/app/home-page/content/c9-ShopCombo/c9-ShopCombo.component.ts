...
@Component({                                                                
  selector: 'app-c9-ShopCombo',                                                
  templateUrl: './c9-ShopCombo.component.html',                                
  styleUrls: ['./c9-ShopCombo.component.scss'],                                
})                                                                          
export class C9ShopcomboComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '806').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '805').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '803').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C9ShopcomboDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c9-ShopCombo-dialog',                            
  templateUrl: 'c9-ShopCombo-dialog.html',                    
  styleUrls: ['./c9-ShopCombo.component.scss'],               
})                                                        
export class C9ShopcomboDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C9ShopcomboDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(801 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
