...
@Component({                                                                
  selector: 'app-c22-Promotion',                                                
  templateUrl: './c22-Promotion.component.html',                                
  styleUrls: ['./c22-Promotion.component.scss'],                                
})                                                                          
export class C22PromotionComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2106').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2105').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '2103').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C22PromotionDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c22-Promotion-dialog',                            
  templateUrl: 'c22-Promotion-dialog.html',                    
  styleUrls: ['./c22-Promotion.component.scss'],               
})                                                        
export class C22PromotionDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C22PromotionDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    this.input.StartDate = this.api.formatDateDOTNET(new Date(this.input.StartDate)); 
    this.input.EndDate = this.api.formatDateDOTNET(new Date(this.input.EndDate)); 

    // check form pass all validate
    if (!this.form.invalid) {
      // if type = 0 insert else update
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(2101 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
