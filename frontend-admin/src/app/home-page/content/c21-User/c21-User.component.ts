...
@Component({
  selector: 'app-c21-User',
  templateUrl: './c21-User.component.html',
  styleUrls: ['./c21-User.component.scss'],
})
export class C21UserComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2006').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  // model
  IdUserStatusModel: any = '0';
  IdRoleUserModel: any = '0';
  ...
  */
   ...
   */
  getListUserstatus() {
    this.subscription.push(this.api.excuteAllByWhat({}, '1800')
      .subscribe((data) => {
        // data = data['result'];   
        data.unshift({ id: '0', Name: 'All' });
        this.UserStatusDatas = data;
      })
    );
  }

  /**                                                            
   * get name display UserStatus by id     
   */
  getDisplayUserstatusById(id) {
    return this.UserStatusDatas.filter((e) => e.id == id)[0]?.Name;
  }
   ...
   */
  getListRoleuser() {
    this.subscription.push(this.api.excuteAllByWhat({}, '1900')
      .subscribe((data) => {
        // data = data['result'];   
        data.unshift({ id: '0', Name: 'All' });
        this.RoleUserDatas = data;
      })
    );
  }

  /**                                                            
   * get name display RoleUser by id     
   */
  getDisplayRoleuserById(id) {
    return this.RoleUserDatas.filter((e) => e.id == id)[0]?.Name;
  }



  /**                                                        
   * on UserStatus Selection Change             
   * @param event                                            
   */
  onUserStatusSelectionChange(event) {
    const condition = { key: "IdUserStatus", value: event.value };

    // add new condition to list                             
    this.addNewConditionToList(condition);
  }
  /**                                                        
   * on RoleUser Selection Change             
   * @param event                                            
   */
  onRoleUserSelectionChange(event) {
    const condition = { key: "IdRoleUser", value: event.value };
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2005').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '2003').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C21UserDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
      data: {
        type: 0,
        id: 0,
        IdUserStatusModel: this.IdUserStatusModel,
        IdRoleUserModel: this.IdRoleUserModel,
 ...
 */
@Component({
  selector: 'c21-User-dialog',
  templateUrl: 'c21-User-dialog.html',
  styleUrls: ['./c21-User.component.scss'],
})
export class C21UserDialog implements OnInit, OnDestroy {
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C21UserDialog>,
    ...
    if (this.type == 1) {
      ...
      // mapping data filter
      this.input.IdUserStatus = data.IdUserStatusModel;
      this.input.IdRoleUser = data.IdRoleUserModel;
  ...
  getDistrictWithCity(id) {
    ...
    this.subscription.push(this.api.excuteAllByWhat(param, '2607').subscribe(data => {
      if (data) {
        data.forEach(element => {
          element.id += '';
        });
    ...
    this.http.post(this.api.IMAGE_UPLOAD_URL, formData, { reportProgress: true, observe: 'events' })
      .subscribe(event => {
        if (event.type === HttpEventType.UploadProgress)
          ...
          this.input.Avatar = this.api.BASEURL + event.body['dbPath'];
        }
      });
  }
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(2001 + this.type) + '')
        .subscribe((data) => {
          if (!data.isError) {
