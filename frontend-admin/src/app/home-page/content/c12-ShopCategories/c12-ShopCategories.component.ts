...
@Component({                                                                
  selector: 'app-c12-ShopCategories',                                                
  templateUrl: './c12-ShopCategories.component.html',                                
  styleUrls: ['./c12-ShopCategories.component.scss'],                                
})                                                                          
export class C12ShopcategoriesComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1106').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1105').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '1103').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C12ShopcategoriesDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c12-ShopCategories-dialog',                            
  templateUrl: 'c12-ShopCategories-dialog.html',                    
  styleUrls: ['./c12-ShopCategories.component.scss'],               
})                                                        
export class C12ShopcategoriesDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C12ShopcategoriesDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(1101 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
