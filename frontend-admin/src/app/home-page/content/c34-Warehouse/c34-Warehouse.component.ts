...
@Component({
  selector: 'app-c34-Warehouse',
  templateUrl: './c34-Warehouse.component.html',
  styleUrls: ['./c34-Warehouse.component.scss'],
})
export class C34WarehouseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '3306').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '3305').subscribe((data) => {
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '3303').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C34WarehouseDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({
  selector: 'c34-Warehouse-dialog',
  templateUrl: 'c34-Warehouse-dialog.html',
  styleUrls: ['./c34-Warehouse.component.scss'],
})
export class C34WarehouseDialog implements OnInit, OnDestroy {
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C34WarehouseDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    this.input.ExpiryDate = this.api.formatDateDOTNET(new Date(this.input.ExpiryDate));
    // check form pass all validate
    if (!this.form.invalid) {
      // if type = 0 insert else update
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(3301 + this.type) + '')
        .subscribe((data) => {
          if (!data.isError) {
