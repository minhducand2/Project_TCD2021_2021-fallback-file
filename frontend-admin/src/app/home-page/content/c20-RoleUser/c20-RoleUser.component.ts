...
@Component({                                                                
  selector: 'app-c20-RoleUser',                                                
  templateUrl: './c20-RoleUser.component.html',                                
  styleUrls: ['./c20-RoleUser.component.scss'],                                
})                                                                          
export class C20RoleuserComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1906').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1905').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '1903').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C20RoleuserDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c20-RoleUser-dialog',                            
  templateUrl: 'c20-RoleUser-dialog.html',                    
  styleUrls: ['./c20-RoleUser.component.scss'],               
})                                                        
export class C20RoleuserDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C20RoleuserDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(1901 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
