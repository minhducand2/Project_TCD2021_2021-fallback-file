...
@Component({                                                                
  selector: 'app-c18-ContactUs',                                                
  templateUrl: './c18-ContactUs.component.html',                                
  styleUrls: ['./c18-ContactUs.component.scss'],                                
})                                                                          
export class C18ContactusComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1706').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
   ...
   */
  loadDataReference() {
    // load list ContactStatus
    this.getListContactstatus();


  }

  /**                                                            
   * get list ContactStatus
   */                                                            
  getListContactstatus() {       
    this.subscription.push(this.api.excuteAllByWhat({}, '1600') 
      .subscribe((data) => {                                     
        // data = data['result'];   
        data.unshift({ id: '0', Name: 'All' });  
        this.ContactStatusDatas = data;  
      })                                                         
    );                                                           
  }                                                              
                                                                 
  /**                                                            
   * get name display ContactStatus by id     
   */                                                            
  getDisplayContactstatusById(id) {                                      
    return this.ContactStatusDatas.filter((e) => e.id == id)[0]?.Name;   
  }                                                              



  /**                                                        
   * on ContactStatus Selection Change             
   * @param event                                            
   */                                                        
  onContactStatusSelectionChange(event) {          
    const condition = { key: "IdContactStatus", value: event.value };  
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1705').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '1703').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C18ContactusDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
      data: {
        type: 0,
        id: 0,
        IdContactStatusModel: this.IdContactStatusModel,
 ...
 */
@Component({                                              
  selector: 'c18-ContactUs-dialog',                            
  templateUrl: 'c18-ContactUs-dialog.html',                    
  styleUrls: ['./c18-ContactUs.component.scss'],               
})                                                        
export class C18ContactusDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C18ContactusDialog>,
    ...
    if (this.type == 1) {
      ...
      // mapping data filter
      this.input.IdContactStatus = data.IdContactStatusModel;
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(1701 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
