...
@Component({                                                                
  selector: 'app-c10-ShopComboDetail',                                                
  templateUrl: './c10-ShopComboDetail.component.html',                                
  styleUrls: ['./c10-ShopComboDetail.component.scss'],                                
})                                                                          
export class C10ShopcombodetailComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '906').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  getListShopcombo() {       
    this.subscription.push(this.api.excuteAllByWhat({}, '800') 
      .subscribe((data) => {                                     
        ...
        this.ShopComboDatas = data;  
      })                                                         
    );                                                           
  }                                                              
                                                                 
  /**                                                            
   * get name display ShopCombo by id     
   */                                                            
  getDisplayShopcomboById(id) {                                      
    return this.ShopComboDatas.filter((e) => e.id == id)[0]?.Name;   
  }                                                              
  ...
  /**                                                        
   * on ShopCombo Selection Change             
   * @param event                                            
   */                                                        
  onShopComboSelectionChange(event) {          
    const condition = { key: "IdShopCombo", value: event.value };  
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '905').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '903').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C10ShopcombodetailDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
      data: {
        type: 0,
        id: 0,
        IdShopComboModel: this.IdShopComboModel,
 ...
 */
@Component({                                              
  selector: 'c10-ShopComboDetail-dialog',                            
  templateUrl: 'c10-ShopComboDetail-dialog.html',                    
  styleUrls: ['./c10-ShopComboDetail.component.scss'],               
})                                                        
export class C10ShopcombodetailDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C10ShopcombodetailDialog>,
    ...
    if (this.type == 1) {
      ...
      // mapping data filter
      this.input.IdShopCombo = data.IdShopComboModel;
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(901 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
