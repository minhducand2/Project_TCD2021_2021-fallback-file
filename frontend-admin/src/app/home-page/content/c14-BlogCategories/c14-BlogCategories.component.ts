...
@Component({                                                                
  selector: 'app-c14-BlogCategories',                                                
  templateUrl: './c14-BlogCategories.component.html',                                
  styleUrls: ['./c14-BlogCategories.component.scss'],                                
})                                                                          
export class C14BlogcategoriesComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1306').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1305').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '1303').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C14BlogcategoriesDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c14-BlogCategories-dialog',                            
  templateUrl: 'c14-BlogCategories-dialog.html',                    
  styleUrls: ['./c14-BlogCategories.component.scss'],               
})                                                        
export class C14BlogcategoriesDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C14BlogcategoriesDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(1301 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
