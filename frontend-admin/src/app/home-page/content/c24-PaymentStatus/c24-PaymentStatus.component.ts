...
@Component({                                                                
  selector: 'app-c24-PaymentStatus',                                                
  templateUrl: './c24-PaymentStatus.component.html',                                
  styleUrls: ['./c24-PaymentStatus.component.scss'],                                
})                                                                          
export class C24PaymentstatusComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2306').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2305').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '2303').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C24PaymentstatusDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c24-PaymentStatus-dialog',                            
  templateUrl: 'c24-PaymentStatus-dialog.html',                    
  styleUrls: ['./c24-PaymentStatus.component.scss'],               
})                                                        
export class C24PaymentstatusDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C24PaymentstatusDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(2301 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
