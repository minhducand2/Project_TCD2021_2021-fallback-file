...
@Component({                                                                
  selector: 'app-c13-MealPlanType',                                                
  templateUrl: './c13-MealPlanType.component.html',                                
  styleUrls: ['./c13-MealPlanType.component.scss'],                                
})                                                                          
export class C13MealplantypeComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1206').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1205').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '1203').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C13MealplantypeDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c13-MealPlanType-dialog',                            
  templateUrl: 'c13-MealPlanType-dialog.html',                    
  styleUrls: ['./c13-MealPlanType.component.scss'],               
})                                                        
export class C13MealplantypeDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C13MealplantypeDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(1201 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
