...
@Component({                                                                
  selector: 'app-c32-MyPromotion',                                                
  templateUrl: './c32-MyPromotion.component.html',                                
  styleUrls: ['./c32-MyPromotion.component.scss'],                                
})                                                                          
export class C32MypromotionComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '3106').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
   ...
   */                                                            
  getListPromotion() {       
    this.subscription.push(this.api.excuteAllByWhat({}, '2100') 
      .subscribe((data) => {                                     
        // data = data['result'];   
        data.unshift({ id: '0', Name: 'All' });  
        this.PromotionDatas = data;  
      })                                                         
    );                                                           
  }                                                              
                                                                 
  /**                                                            
   * get name display Promotion by id     
   */                                                            
  getDisplayPromotionById(id) {                                      
    return this.PromotionDatas.filter((e) => e.id == id)[0]?.Name;   
  }                                                              



  /**                                                        
   * on Promotion Selection Change             
   * @param event                                            
   */                                                        
  onPromotionSelectionChange(event) {          
    const condition = { key: "IdPromotion", value: event.value };  
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '3105').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '3103').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C32MypromotionDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
      data: {
        type: 0,
        id: 0,
        IdPromotionModel: this.IdPromotionModel,
 ...
 */
@Component({                                              
  selector: 'c32-MyPromotion-dialog',                            
  templateUrl: 'c32-MyPromotion-dialog.html',                    
  styleUrls: ['./c32-MyPromotion.component.scss'],               
})                                                        
export class C32MypromotionDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C32MypromotionDialog>,
    ...
    if (this.type == 1) {
      ...
      // mapping data filter
      this.input.IdPromotion = data.IdPromotionModel;
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(3101 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
