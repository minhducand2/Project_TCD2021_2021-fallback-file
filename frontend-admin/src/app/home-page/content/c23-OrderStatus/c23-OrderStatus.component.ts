...
@Component({                                                                
  selector: 'app-c23-OrderStatus',                                                
  templateUrl: './c23-OrderStatus.component.html',                                
  styleUrls: ['./c23-OrderStatus.component.scss'],                                
})                                                                          
export class C23OrderstatusComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2206').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2205').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '2203').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C23OrderstatusDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c23-OrderStatus-dialog',                            
  templateUrl: 'c23-OrderStatus-dialog.html',                    
  styleUrls: ['./c23-OrderStatus.component.scss'],               
})                                                        
export class C23OrderstatusDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C23OrderstatusDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(2201 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
