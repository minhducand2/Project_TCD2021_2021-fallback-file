...
@Component({                                                                
  selector: 'app-c17-ContactStatus',                                                
  templateUrl: './c17-ContactStatus.component.html',                                
  styleUrls: ['./c17-ContactStatus.component.scss'],                                
})                                                                          
export class C17ContactstatusComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1606').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '1605').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '1603').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C17ContactstatusDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c17-ContactStatus-dialog',                            
  templateUrl: 'c17-ContactStatus-dialog.html',                    
  styleUrls: ['./c17-ContactStatus.component.scss'],               
})                                                        
export class C17ContactstatusDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C17ContactstatusDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(1601 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
