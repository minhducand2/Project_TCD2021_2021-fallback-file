...
@Component({
  selector: 'app-c7-HeaderInfo',
  templateUrl: './c7-HeaderInfo.component.html',
  styleUrls: ['./c7-HeaderInfo.component.scss'],
})
export class C7HeaderinfoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '606').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '605').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '603').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C7HeaderinfoDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({
  selector: 'c7-HeaderInfo-dialog',
  templateUrl: 'c7-HeaderInfo-dialog.html',
  styleUrls: ['./c7-HeaderInfo.component.scss'],
})
export class C7HeaderinfoDialog implements OnInit, OnDestroy {
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C7HeaderinfoDialog>,
    ...
    this.http.post(this.api.IMAGE_UPLOAD_URL, formData, { reportProgress: true, observe: 'events' })
      .subscribe(event => {
        if (event.type === HttpEventType.UploadProgress)
          ...
          this.input.Logo = this.api.BASEURL + event.body['dbPath'];
        }
      });
  }
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      if (this.input.Logo == '') {
        this.api.showWarning('Vui lòng upload logo');
        this.isUpdate = false;
      } else {
        // if type = 0 insert else update
        this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(601 + this.type) + '')
          .subscribe((data) => {
            if (!data.isError) {
