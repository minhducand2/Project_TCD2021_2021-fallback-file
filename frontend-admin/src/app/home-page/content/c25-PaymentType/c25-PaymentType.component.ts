...
@Component({                                                                
  selector: 'app-c25-PaymentType',                                                
  templateUrl: './c25-PaymentType.component.html',                                
  styleUrls: ['./c25-PaymentType.component.scss'],                                
})                                                                          
export class C25PaymenttypeComponent implements OnInit, AfterViewInit, OnDestroy {  
  ...
  getLengthOfPage() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2406').subscribe((data) => {
        // data = data['result']; 
        if (data.length > 0) {
  ...
  */
  ...
  onLoadDataGrid() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2405').subscribe((data) => {
        // data = data['result']; 
        if (data.length>0) {
          // set data for table
  ...
  onBtnDelClick() {
    ...
    if (listId !== '') {
      this.subscription.push(
        this.api.excuteAllByWhat(param, '2403').subscribe((data) => {
          if (!data.isError) {
            // load data grid
            this.onLoadDataGrid();
  ...
  onBtnInsertDataClick() {
    const dialogRef = this.dialog.open(C25PaymenttypeDialog, {
      width: '100%',
      height: '100%',
      maxWidth: '100%',
      maxHeight: '100%',
 ...
 */
@Component({                                              
  selector: 'c25-PaymentType-dialog',                            
  templateUrl: 'c25-PaymentType-dialog.html',                    
  styleUrls: ['./c25-PaymentType.component.scss'],               
})                                                        
export class C25PaymenttypeDialog implements OnInit, OnDestroy {  
   ...
   */
  constructor(
    public dialogRef: MatDialogRef<C25PaymenttypeDialog>,
  ...
  onBtnSubmitClick(): void {
    ...
    if (!this.form.invalid) {
      ...
      this.subscription.push(this.api.excuteAllByWhat(this.input, '' + Number(2401 + this.type) + '')
      .subscribe((data) => {
        if (!data.isError) {
