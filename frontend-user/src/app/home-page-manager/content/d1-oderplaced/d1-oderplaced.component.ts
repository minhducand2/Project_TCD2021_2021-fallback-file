...
@Component({
  selector: 'app-d1-oderplaced',
  templateUrl: './d1-oderplaced.component.html',
  styleUrls: ['./d1-oderplaced.component.scss']
})
export class D1OderPlacedComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  getLoadDataUserOder() {
    ...
    this.subscription.push(
      this.api.excuteAllByWhat(param, '2818').subscribe((result) => {
        let stt = 1;
        if (result.length > 0) {
...
*/
   ...
   */
  onViewsClick(id) {
    const url = '/manager/admin/d2-order-views/' + id + '/' + '1' + '/orderview';
  ...
  onClickCancelOrder(item) {
    this.idCancelOrder = item.id;
  }

  onBtnDelClick() {
    const param = {
      id: this.idCancelOrder
    }
    this.subscription.push(this.api.excuteAllByWhat(param, '2822')
      .subscribe((result) => {
        if (result) {
